...
**操作步骤（助手执行）**：
1. 用 notion read 读取该子页的全部 to_do 块的 `checked` 布尔值与文本。
2. 读 `TODO.md`，构建「任务文字 → 行号」索引。
3. 计算差异：
   - `checked=true` 且 git 行是 `[ ]` → 改为 `[x]`
   - `checked=false` 且 git 行是 `[x]` → 改回 `[ ]`（若用户取消勾选）
   - Notion 有、git 没有的新增任务行 → 追加到最贴合的分类小标题下
4. 用 edit 改 `TODO.md`，仅动有差异的行，不重排版、不动已完成历史的无关行。
5. optionally `git add TODO.md && git commit -m "docs(TODO): sync from notion board"`（提交前让用户知情即可）。
