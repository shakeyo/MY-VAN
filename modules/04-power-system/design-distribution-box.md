...
### 6.1 485 继电器模块（当前：8路箱内）

当前阶段只上 8 路模块。4 路模块后期做加法（485 并线、电源从控制区预留端子取，零拆改）。
...
> 7/8 路使用，R3 空闲。LED 当前不接，12 芯线相关芯预留。
...
> 不上位机，ESPhome 在 ESP32 本地跑自动化。后期加 HA 只做 dashboard，不改线。
...
| 位 | 负载 | 来自 | 线径 |
|----|------|------|------|
| 1 | 点烟器1+ | F12 直连（不经继电器） | 2.5mm² |
| 2 | 点烟器2+ | F12 直连（不经继电器） | 2.5mm² |
| 3 | 冰箱+ | 中8.R8.NO | 1.5mm² |
| 4 | 主灯+ | 中8.R1.NO | 0.75mm² |
| 5-8 | 氛围灯/睡眠灯+ | 中8.R2.NO，4 位并联供电 | 各路 0.75mm² |
| 9 | 倒车灯+ | 中8.R5.NO | 0.75mm² |
| 10 | 后窗排气扇+ | 中8.R4.NO | 1.0mm² |
| 11 | 电控水阀+ | 中8.R7.NO | 0.75mm² |
| 12 | 预留 | — | — |
