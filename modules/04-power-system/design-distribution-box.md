...
| 中部 | 负极端子排 | 灰端子 + 短接片桥接，暂代铜排；全系统负极汇流点 |
| 右下 | 8 路 485 继电器模块 | 逻辑控制 + 功率切换（详见 switch-assignment.md） |
| 底部 | 端子排 | 功率区（正极独立 + 负极桥接）+ 控制区 |
...
| 保险位 | 设备 | 电流 | 线径 | 负极回哪 |
|--------|------|------|------|----------|
| F12-1 | 控制系统（8路模块+485+面板背光） | 3A | 0.75mm² | 负极端子排（桥接） |
| F12-2 | 主灯 | 5A | 0.75mm² | 负极端子排（桥接） |
| F12-3 | 氛围灯 + 睡眠灯 | 5A | 0.75mm² | 负极端子排（桥接） |
| F12-4 | 后窗排气扇 | 7.5A | 1.0mm² | 负极端子排（桥接） |
| F12-5 | 倒车灯 | 5A | 0.75mm² | 负极端子排（桥接） |
| F12-6 | 冰箱 | 10A | 1.5mm² | 负极端子排（桥接） |
| F12-7 | 电控水阀（排水） | 5A | 0.75mm² | 负极端子排（桥接） |
| F12-8 | 车内锁 | 5A | 0.75mm² | 负极端子排（桥接） |
| F12-9 | 踏步控制信号（外接继电器线圈） | 3A | 0.75mm² | 负极端子排（桥接） |
| F12-10 | 点烟器 | 15A | 2.5mm² | 负极端子排（桥接） |
| F12-11~12 | 备用/扩展 | — | — | — |
...
### 6.1 485 继电器模块（8路箱内 + 4路柜内）

16 路模块尺寸过大塞不进配电箱，改用现有 8 路 + 4 路分开放置。两个模块挂同一条 485 总线，均由 F12-1 供电（12V/GND）。

**8 路模块（配电箱内）**：扛所有功率回路。详细通道分配见 [switch-assignment.md](switch-assignment.md)。

| 通道 | 职能 | 说明 |
|------|------|------|
| R1-R2, R4-R8 | 负载继电器 | 主灯/氛围灯+睡眠灯/后窗排气扇/倒车灯/上水泵(接触器)/电控水阀/冰箱 |
| R3 | 空载 DO | 排气扇场景 LED 驱动 |
| DI1-DI4 | 入口面板按键 | 排水/户外灯/上水泵/离车驻车 |
| DI5-DI8 | 主控面板键1-4 | 主灯/氛围灯/排气扇/上水泵 |

> 8 路打满，零余量。R3 空载仅驱动 LED。

**4 路模块（中门柜内，主控面板旁）**：零功率线，仅跑信号。

| 通道 | 职能 | 说明 |
|------|------|------|
| R1-R3 | 空载 DO | 离车/驻车 LED / 灯光模式 LED / 户外灯 LED（跨模块软件同步） |
| R4 | 预留 | 后期扩展 |
| DI1-DI3 | 主控面板键5-7 | 冰箱/灯光模式/预留 |
...
三个面板的按键/LED 分配及 12 芯线色定义详见 [switch-assignment.md](switch-assignment.md)。

- **入口面板**（4 键）→ 12芯A → 箱内 8 路 DI1-4
- **主控面板 键1-4**（主灯/氛围灯/排气扇/上水泵）→ 12芯B → 箱内 8 路 DI5-8
- **主控面板 键5-6**（冰箱/灯光模式）→ 细线束C → 柜内 4 路 DI1-2
- 主控键5（冰箱）LED 例外：走 12芯B 回箱接 8路.OUT8 实现硬件同步
...
| 位 | 负载 | 来自 | 线径 |
|----|------|------|------|
| 1 | 点烟器+ | F12-10 直连（不经继电器） | 2.5mm² |
| 2 | 冰箱+ | 中8.R8.NO | 1.5mm² |
| 3 | 主灯+ | 中8.R1.NO | 0.75mm² |
| 4 | 氛围灯/睡眠灯+ | 中8.R2.NO | 2.5mm² |
| 5 | 后窗排气扇+ | 中8.R4.NO | 1.0mm² |
| 6 | 倒车灯+ | 中8.R5.NO | 0.75mm² |
| 7 | 电控水阀+ | 中8.R7.NO | 0.75mm² |
| 8 | 备用 | — | — |

> 位 4 出一根 2.5mm² 干线 → WAGO 2进5出 → 分接氛围灯带+ / 睡眠灯带+。
...
> 位 9-18 用 UK 短接片连成一体 → 引 6mm² 黑线到电池总负。后期买到负极铜排后迁移。
