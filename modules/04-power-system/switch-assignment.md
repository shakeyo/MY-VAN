...
> **模块丝印对照**：中8 模块实物端子按 PLC 习惯标 **X/Y**——**X1-X8 = 数字量输入**（即本文 DI1-8），**Y1-Y8 = 继电器输出**（即本文 R1-R8）。接线/排查按丝印找端子，按本文名称对逻辑：如 中8.R1 = 丝印 **Y1**、中8.DI1 = 丝印 **X1**。每个 Y 是继电器干接点，**COM = 公共端（进电侧）**，**NO = 常开出线到负载**，NC 本设计不用。
...
> **双控 LED 汇接**：入口键3 与 主控键4 双控上水泵（中8.R6），两颗按钮的 LED 阴极（14芯A 芯11 粉、14芯B 芯12 橙）都进 `中8.OUT6`（R6 硬件同步输出，一起亮一起灭）。两根线在配电箱端子排「上水泵LED」位汇接后单线进 OUT6；LED 指示线独立，不与泵功率线并接。
...
2. 该并的先并好，套热缩管 + 贴标签：
   ...
   - 双控泵 LED：芯11粉 + 芯12橙 在「上水泵LED」位并好，出单线到 中8.OUT6
